#扫码获取爱奇艺P00001, 此二维码有效期只有半分钟,但是爱奇艺没给二维码失效判断,所以默认执行6次后未完成扫码就结束,二维码生成在工作目录里
#实测此方法获取的P00001有效期为三个月,如果只用于签到,超过三个月之后还能用,但是无法碰蛋和查询信息
...
'surl': 1
    ...
    return post(url, headers=headers, data=data).cookies
def qrcode(token):
