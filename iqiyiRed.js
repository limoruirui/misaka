...
方法二 不限制： 网页打开 https://iqiyi.ruirui.fun/login 按提示进行扫码登录 将获取到的ck填入以下脚本变量或环境变量即可 青龙环境变量 IQIYI_COOKIE
...
[task_local]
# 爱奇艺会员签到
0 9 * * * https://raw.githubusercontent.com/limoruirui/misaka/master/iqiyiRed.js
[rewrite_local]
# 获取Cookie
...
**********************
[Script]
爱奇艺领取会员红包 = type=cron,cronexp=0 9 * * *,timeout=120,script-path=https://raw.githubusercontent.com/limoruirui/misaka/master/iqiyiRed.js
...
function genRedNo() {
    return new Promise(resolve => {
        ...
        $nobyda.get(URL, function(error, response, data) {
          const redNo = data.code === "A00000" ? data.data.redNo : " "
          if (redNo) {
            $nobyda.redNo = redNo;
            console.log(`本次生成的红包码为${redNo},正在检测合法性......`);
            result = await checkRedNo(redNo);
            if (result === "A00000") {
                console.log("检测到本次生成的红包码合法，正在将其上传到数据库供大家领取");
                res = await postRedNo(redNo);
                if (res === "success") {
...
function getRed() {
  return new Promise(resolve => {
      ...
      var URL = {
        url: 'https://act.vip.iqiyi.com/bonus/api/grabRed?accountType=2&P00001=' + P00001 + `&_=${timestamp}&callback=Zepto${timestamp}&redNo=` + redNo,
      }
      $nobyda.get(URL, function(error, response, data) {
        if (data.code === "A00000") {
          const days = data.data.receiveDays;
          console.log(`领取成功，本次领取到${days}天`);
        } else {
          console.log(`领取失败，原因是${data.msg}`);
        }
        resolve();
      })
    }})
...
function postRedNo(redNo) {
    return new Promise(resolve => {
        ...
        var URL = {
          url: "https://iqiyi.ruirui.fun/postRedNo",
          headers: {
...
function getRedNo() {
  return new Promise(resolve => {
      var URL = {
        url: 'https://iqiyi.ruirui.fun/getRedNo',
      }
      $nobyda.get(URL, function(error, response, data) {
        if (data.msg === "success") {
          const redNoList = data.data;
          resolve(redNoList);
        } else {
