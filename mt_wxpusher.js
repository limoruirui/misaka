/*
cron: 2 10-12,15-18 * * *
const $ = new Env('美团优惠券查询');
*/


/*========================修改数据区域===========================*/

//你wxpusher的token，如AT_ZzQKKQF9xxx
const appToken = "填你wxpusher的token";
//你wxpusher的topicIds，如['11540']
const topicIds = [];
//你wxpusher的uids,如["UID_XXX"]
const uids = [];
//需要筛选的青龙环境数据类型
const cookieName = ["MT25_12","MT25_12_ONE","MT25_12_TWO","MT30_15","MT30_15_ONE","MT30_15_TWO","cookie1"];
//你需要筛选的优惠券金额关键字
const keywords = ['30', '40', '16', '18', '15','12', '13', '25','26'];
...
const couponKeywords = ['神券'];
const matchedCoupons = [];
let cookies = [];
const baseUrl = "https://i.meituan.com/account/magiccard";

!(async () => {
    //导入环境文件
    try {
        require.resolve("./ql_api");
        const {
            getEnvs
        } = require('./ql_api');
        // 存在
        ...
        for(let i = 0; i < cookieName.length; i++){
            let envName = cookieName[i];
            // console.log(envName);
            const envs = await getEnvs(envName)
            // console.log(envs);
            await Envs(envName,envs);
        }
    } catch(e) {
        // 不存在
        const {
            getEnvs
        } = require('./tools/ql_api');
        // 读取环境变量，动态添加cookies
        for(let i = 0; i < cookieName.length; i++){
            let envName = cookieName[i];
            const envs = await getEnvs(envName)
...
// cookie.status == 0代表青龙启用这个CK
  const enabledCookies = cookies.filter(cookie => cookie.status == 0);
//   console.log(enabledCookies);
  const promises = enabledCookies.map(async cookie => {
    //   console.log(cookie);
    const headers = {
      Cookie: cookie.value
    };
    ...
    try {
      ...
      $('.moneycard').each((i, el) => {
        const moneycardValue = $(el).find('.moneycard-value').text().trim();
        const moneycardDes = $(el).find('.moneycard-des').text().trim();
        const moneycardDesc = $(el).find('.moneycard-desc h6').text().trim();
        const moneycardStatus = $(el).find('.moneycard-status span').text().trim();

        if (moneycardStatus.includes('已过期')) {
          return;
        }

        const expirationDate = moneycardStatus.match(/\d{4}-\d{2}-\d{2}/)[0];
        if (moneycardStatus.includes('已使用')) {
          if (expirationDate !== dateString) {
            return;
          }
        }

        for (const keyword of keywords) {
            if (moneycardValue.includes(keyword)) {
                for (const couponKeyword of couponKeywords) {
                    if (moneycardDesc.includes(couponKeyword)) {
                        let couponDescription = moneycardDes.split('。')[0] + '。';
                        let total = couponDescription.replace(/[^\d.]/g, "");
                        let available = moneycardValue.replace(/[^\d.]/g, "");
                        let couponCondition = total+"-"+available;
                        const couponInfo = {
                            cookieName: cookie.remarks,
                            couponName: moneycardDesc,
                            couponCondition:couponCondition,
                            couponValue: moneycardValue,
                            couponDescription: couponDescription,
                            couponStatus: moneycardStatus
                        };

                        // 判断是否当天使用过的或未使用的优惠券
                        const expirationDate = moneycardStatus.match(/\d{4}-\d{2}-\d{2}/)[0];
                        const isToday = expirationDate === dateString;
                        const isUsed = moneycardStatus.includes('已使用');

                        if (isToday || !isUsed) {
                            matchedCoupons.push(couponInfo);
                        }
  ...
  console.log(matchedCoupons);

  await sendCouponsToWeChat();
}

async function sendCouponsToWeChat() {
  const formattedCoupons = matchedCoupons.map(coupon => {
    return `<hr/><br/>账号: ${coupon.cookieName}<br/>优惠券名称: ${coupon.couponName}<br/>优惠券条件: ${coupon.couponCondition}<br/>优惠券金额: ${coupon.couponValue}<br/>优惠券描述: ${coupon.couponDescription}<br/>优惠券状态: ${coupon.couponStatus}<br/>`;
  });
  ...
  const body = {
    "appToken": appToken,
    "content": JSON.stringify(formattedCoupons).replace(/\["/g, '').replace(/\","/g, '').replace(/\"]/g, ''),
    "summary": "美团红包列表推送",
    "contentType": 2,
    "topicIds": topicIds,
    "uids": uids,
  ...
  try {
    const response = await axios.post("https://wxpusher.zjiecode.com/api/send/message", body, { headers });
...
//     console.log(key);
//     if (key.startsWith('MT25') || key.startsWith('MT30') || key == "COOKIE_DICT") {
        
//         cookies.push({ name: key, value: process.env[key], enabled: 1 });
//     }
// });

function getRepeatNum(arr){ //计算重复值
    ...
    for (var i = 0; i < arr.length; i++) {
        if (temp.indexOf(arr[i].remarks) === -1) {
            temp.push(arr[i].remarks);
            array.push(arr[i])
        }
    }
    return array;
...
//脚本环境变量获取
async function Envs(name,ckList) {
    if (ckList) {
        // 
        try {
            ckList.forEach(key =>{
